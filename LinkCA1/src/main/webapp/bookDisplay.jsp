...
body {
	/* background: url("/src/wooden-table-with-blurred-background.jpg") no-repeat bottom center, linear-gradient(to top, #000 25%, transparent); */
	background: #111827;
	/* min-height: 100vh; */
}
...
#detailsGrid {
	display: grid;
	grid-template-columns: minmax(0, 125px) 1fr;
}
			...
			<div class="w-4/5 text-white">
				<div class="flex mt-6">
					...
					<div class="ml-5 flex flex-col w-full  pr-5">
						...
						<form class=""
							action="<%=request.getContextPath()%>/AddToBasket?title=<%=book.getTitle()%>&price=<%=book.getPrice()%>&imageURL=<%=book.getImageURL()%>&quantity="
